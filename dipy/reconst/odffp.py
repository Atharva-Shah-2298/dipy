...
class DiffusionDataGenerator(object):
    ...
    def _validate_fraction_volumes(self, p_iso, p_fib):
        ...
        if p_iso[0] + self.max_peaks_num * p_fib[0] >= 1:
            raise Exception(
                "Lower boundaries of fraction volumes are too high for "
                "max_peaks_num=%d" % self.max_peaks_num
            )
    ...
    def _random_fraction_volumes(self, p_iso, p_fib, peaks_per_voxel):
        ...
        # Lower bounds are hard limits, so the variability remains between 0
        # and p_random_max
    ...
    def _random_micro_parameters(self, f_in, D_iso, D_a, D_e, D_r,
                                 peaks_per_voxel, equal_fibers,
                                 assert_faster_D_a, tortuosity_approximation):
        ...
        while True:
            # Equal fibers means that all fibers have the same diffusivities
            # and f_in
            if equal_fibers:
            ...
            if assert_faster_D_a and \
                    np.any(micro_params[self.MICRO_DA, 1:] <
                           micro_params[self.MICRO_DE, 1:]):
                continue

            if tortuosity_approximation:
                micro_params[self.MICRO_DR, 1:] = (
                    1 - micro_params[self.MICRO_FIN, 1:]) * \
                    micro_params[self.MICRO_DA, 1:]
                if np.any(micro_params[self.MICRO_DR, 1:] < D_r[0]) or \
                        np.any(micro_params[self.MICRO_DR, 1:] > D_r[1]):
                    continue
    ...
    def _compute_dwi(self, ratio, micro, peak_dirs_idx):
        ...
        for j in range(len(peak_dirs_idx)):
            ...
            dir_prod_sqr = np.dot(
                self.gtab.bvecs, self.tessellation.vertices[peak_dirs_idx[j]].T) \
                ** 2

            dwi_intra = np.exp(-bvals *
                               micro[self.MICRO_DA, j+1] * dir_prod_sqr)
            dwi_extra = np.exp(-bvals * (micro[self.MICRO_DE, j+1] *
                               dir_prod_sqr + micro[self.MICRO_DR, j+1] *
                                         (1 - dir_prod_sqr)))

            dwi += ratio[j+1] * (micro[self.MICRO_FIN, j+1] *
                                 dwi_intra + (1 - micro[self.MICRO_FIN, j+1]) *
                                 dwi_extra)
...
class OdffpDictionary(DiffusionDataGenerator):
    ...
    def _sort_peaks(self):
        for j in range(self.peak_dirs.shape[2]):
            ...
            # First, convert spherical coordinates from Matlab
            # (azim, elev, radius) to Python (radius, phi, theta)
            # and then to Cartesian coordinates
            peak_dirs = np.array(sphere2cart(
                1, np.pi/2 + self.peak_dirs[1, :, j], self.peak_dirs[0, :, j]))
            peaks_filter = np.any(~np.isnan(peak_dirs), axis=0)

            if ~np.any(peaks_filter):
                continue

            peak_vertex_idx = np.argmax(
                np.dot(self.tessellation.vertices, peak_dirs[:, peaks_filter]),
                axis=0)
            peak_vertex_values = self.odf[np.mod(
                peak_vertex_idx, self.odf.shape[0]), j]
    ...
    def _peaks_per_voxel_cdf(self, total_dirs_num):
        """Cummulative Distribution Function (CDF) of a random variable:
        peaks_per_voxel"""

        # Numbers of directions are in the proportion 1: 1*(k-1) : 1*(k-1)*(k-2)
        # Thus, for k=321 (total_dirs_num) the cumulative number of directions
        # is [1,321,102401,...]
        ...
        # Compute the cumulative number of directions for fibers
        # other than [0,0,1]
    ...
    def load(self, dict_file):
        with h5py.File(dict_file, 'r') as mat_file:
            ...
            self.max_peaks_num = np.sum(
                np.any(~np.isnan(peak_dirs[0]), axis=1))
        ...
        # Compute the number of peaks per voxel
        # as the number of non-NaN fiber directions per voxel
        self.peaks_per_voxel = np.sum(
            ~np.isnan(self._dict.peak_dirs[0, :, :]), axis=0)
    ...
    def append(self, external_odf_dict):
        ...
        self.peak_dirs = np.concatenate(
            (self.peak_dirs, external_odf_dict.peak_dirs), axis=2)
        self.micro = np.concatenate(
            (self.micro, external_odf_dict.micro), axis=2)
        self.ratio = np.concatenate(
            (self.ratio, external_odf_dict.ratio), axis=1)
        self.peaks_per_voxel = np.concatenate(
            (self.peaks_per_voxel, external_odf_dict.peaks_per_voxel))

    def generate(self, dict_size=1000000, max_peaks_num=3, equal_fibers=False,
                 p_iso=[0.0, 1.0], p_fib=[0.0, 1.0], f_in=[0.0, 1.0],
                 D_iso=[2.0, 3.0], D_a=[1.5, 2.5], D_e=[1.5, 2.5],
                 D_r=[0.5, 1.5], max_chunk_size=10000, odf_recon_model=None,
        ...
        f_in, D_iso, D_a, D_e, D_r = self._validate_micro_parameters(
            f_in, D_iso, D_a, D_e, D_r)
        ...
        # VOID element of the ODF-dictionary represents
        # empty voxels outside mask
        # no isotropic water
        self.ratio[0, self.IDX_VOID] = 0
        # no diffusivity at all
        self.micro[self.MICRO_DE, 0, self.IDX_VOID] = 0
        # to be skipped in ODF matching, hence peaks_per_voxel < 0
        self.peaks_per_voxel[self.IDX_VOID] = -1

        # ISO element of the ODF-dictionary represents voxels
        # with isotropic water only
        # isotropic water without fibers, hence p_iso=1
        self.ratio[0, self.IDX_ISO] = 1
        # diffusivity of free water at 37C
        self.micro[self.MICRO_DE, 0, self.IDX_ISO] = 3
        # no fibers, hence peaks_per_voxel=0
        self.peaks_per_voxel[self.IDX_ISO] = 0
        self.odf[:, self.IDX_ISO] = np.squeeze(
            self._compute_odf_trace(
                odf_recon_model, self.ratio[:, self.IDX_ISO],
                self.micro[:, :, self.IDX_ISO], [])
        )

        for chunk_idx in np.split(range(self.PREDEFINED_IDX_NUM, dict_size),
                                  range(max_chunk_size, dict_size,
                                        max_chunk_size)):
            ...
            peak_dirs_idx = np.zeros(
                (self.max_peaks_num, chunk_size), dtype=int)

            # Draw the numbers of peaks per voxel randomly.
            # The direction [0,0,1] is obligatory, hence 1 + np.sum(...)
            self.peaks_per_voxel[chunk_idx] = 1 + np.sum(
                np.random.uniform(size=(chunk_size, 1)) >
                self._peaks_per_voxel_cdf(total_dirs_num), axis=1
            )
            ...
            for i, j in zip(range(chunk_size), chunk_idx):
                ...
                peak_dirs_idx[1:self.peaks_per_voxel[j], i] = np.random.choice(
                    range(1, total_dirs_num), self.peaks_per_voxel[j]-1,
                    replace=False
                )
                ...
                self.peak_dirs[:, :self.peaks_per_voxel[j], j] = np.array([
                    ...
                    self.tessellation.theta[peak_dirs_idx[:self.peaks_per_voxel[j], i]] -
                    np.pi/2
            ...
            for i, j in zip(range(chunk_size), chunk_idx):
                ...
                if np.any(sorted_idx != seq_idx):
                    self.micro[:, seq_idx+1,
                               j] = self.micro[:, sorted_idx+1, j]
                    self.ratio[seq_idx+1, j] = self.ratio[sorted_idx+1, j]

                # If the highest peak was not at [0,0,1],
                # recompute the ODF with the reordered parameters.
                # Note that peak_dirs_idx[0] = 0,
                # so it's sufficient to test if sorted_idx[0] != 0
                if sorted_idx[0] != 0:
...
class PosteriorOdffpDictionary(OdffpDictionary):
    ...
    def __init__(self, gtab, odffp_fit, tessellation=dsiSphere8Fold()):
        ...
        for peaks_num in range(1, odffp_fit.get_max_peaks_num()+1):
            ...
            try:
                ...
                # Fit KDE model of the compartment volumes
                self._ratio_pdf[peaks_num] = scipy.stats.gaussian_kde(
                    compartment_volumes[1:])

            except:
                # If not succeeded, take mean values
                self._ratio_pdf[peaks_num] = np.mean(
                    compartment_volumes, axis=1)
            ...
            for peak_id in range(peaks_num+1):
                ...
                micro_parameters = odffp_fit.get_micro_parameters(
                    peaks_num, peak_id)
                try:
                    ...
                    # If not succeeded, take mean values
                    self._micro_pdf[peaks_num][peak_id] = np.mean(
                        micro_parameters, axis=1)
    ...
    def _random_fraction_volumes(self, p_iso, p_fib, peaks_per_voxel):
        try:
            fiber_fraction_volumes = np.squeeze(
                self._ratio_pdf[peaks_per_voxel].resample(1))
            water_fraction_volume = np.maximum(
                p_iso[0], 1 - np.sum(fiber_fraction_volumes))
    ...
    def _random_micro_parameters(self, f_in, D_iso, D_a, D_e, D_r, peaks_per_voxel,
        ...
        while True:
            ...
            if tortuosity_approximation:
                micro_params[self.MICRO_DR, 1:] = (
                    1 - micro_params[self.MICRO_FIN, 1:]) * micro_params[self.MICRO_DA, 1:]
...
class PosteriorOdffpDictionaryFromFib(PosteriorOdffpDictionary):
    ...
    def __init__(self, gtab, fib_file_name, tessellation=dsiSphere8Fold()):
        ...
        all_compartment_volumes = np.zeros(
            (max_peaks_num+1, np.sum(voxel_filter)))
        all_micro_parameters = np.zeros(
            (max_peaks_num+1, OdffpDictionary.MICRO_PARAMS_NUM, np.sum(voxel_filter)))
        ...
        peaks_per_voxel = np.sum(
            ~np.isnan(all_compartment_volumes[1:]), axis=0)

        for peaks_num in range(1, max_peaks_num+1):

            peaks_num_filter = self._get_peaks_num_filter(
                peaks_num, peaks_per_voxel)

            compartment_volumes = all_compartment_volumes[:peaks_num +
                                                          1, peaks_num_filter]
            try:
                # Fit KDE model of the compartment volumes
                self._ratio_pdf[peaks_num] = scipy.stats.gaussian_kde(
            ...
            for peak_id in range(peaks_num+1):
                ...
                micro_parameters = all_micro_parameters[peak_id,
                                                        :, peaks_num_filter].T
                try:
                    # Fit KDE model of the microstructure parameters
                    self._micro_pdf[peaks_num][peak_id] = scipy.stats.gaussian_kde(
...
class OdffpModel(object):
    ...
    def resample_odf(odf, in_sphere, out_sphere):
        ...
        return np.squeeze(
            sh_to_sf(sf_to_sh(odf, in_sphere), out_sphere)[
                :, :sphere_half_size]
        )
    ...
    def _find_highest_peak_rotation(self, input_odf, target_dir=[0, 0, 1]):
        rotation = np.eye(3)
        input_peak_dirs, _, _ = peak_directions(
            input_odf, self._dict.tessellation)
        ...
        if len(input_peak_dirs) > 0:
            ...
            if sum_sqr_cr != 0:
                s = np.array(
                    [[0, -cr[2], cr[1]], [cr[2], 0, -cr[0]], [-cr[1], cr[0], 0]])
                rotation += s + \
                    np.dot(s, s) * (1 - np.dot(highest_peak_dir,
                                               target_dir)) / sum_sqr_cr
    ...
    def _rotate_peak_dirs(self, peak_dirs, rotation):
        return np.dot(
            np.array(sphere2cart(1, np.pi/2 +
                     peak_dirs[1, :], peak_dirs[0, :])).T,
            rotation
        )

    def _unmask(self, vector, mask):
        output_matrix = np.zeros(
            mask.shape + vector.shape[1:], dtype=vector.dtype)
    ...
    def _find_matching_odf_trace(self, input_odf_trace, dict_odf_trace, penalty):
        ...
        try:
            ...
            max_dot_product_values = np.zeros(
                (self._dict.max_peaks_num+1, input_odf_trace_num))
            max_dot_product_dict_idx = np.zeros(
                (self._dict.max_peaks_num+1, input_odf_trace_num), dtype=int)
            ...
            for peak_id in range(self._dict.max_peaks_num+1):
                ...
                peak_filter_idx = np.arange(
                    len(self._dict.peaks_per_voxel))[peak_filter]

                dot_product = np.dot(
                    input_odf_trace, dict_odf_trace[:, peak_filter])
    ...
    def fit(self, data, mask=None, max_chunk_size=1000, penalty=DEFAULT_FIT_PENALTY):
        ...
        for chunk_idx in np.split(range(voxels_num), range(max_chunk_size, voxels_num, max_chunk_size)):
            ...
            input_odf = self._odf_recon_model.fit(
                masked_data[chunk_idx]).odf(self._dict.tessellation)
            ...
            for i in range(chunk_size):
                ...
                rotated_tessellation[i] = self._rotate_tessellation(
                    self._dict.tessellation, rotation[i])

                input_odf_trace[i], input_odf_norm[i] = self._normalize_odf(
                    self.resample_odf(
                        input_odf[i], self._dict.tessellation, rotated_tessellation[i])
                )

            dict_idx[chunk_idx] = self._find_matching_odf_trace(
                input_odf_trace, dict_odf_trace, penalty)
            ...
            for i, j in zip(range(chunk_size), chunk_idx):
                ...
                output_peak_dirs[j] = self._rotate_peak_dirs(
                    self._dict.peak_dirs[:, :, dict_idx[j]], rotation[i])
...
class OdffpFit(OdfFit):
    ...
    def _export_dict_var(self, dict_var, fixed_peaks_num=None, peak_id=None, full_volume=False):
        ...
        if peak_id and peak_id > self._dict.max_peaks_num:
            raise Exception(
                "Argument peak_id=%d exceeds the maximum number of peaks." % peak_id)
        ...
        if full_volume:
            ...
            if fixed_peaks_num and np.any(self._dict.peaks_per_voxel[select_dict_idx] == fixed_peaks_num):
                select_dict_idx = select_dict_idx[self._dict.peaks_per_voxel[select_dict_idx]
                                                  == fixed_peaks_num]
    ...
    def odf(self, sphere=None):
        if sphere is None or sphere == self._dict.tessellation:
            ...
            output_odf = OdffpModel.resample_odf(
                self._odf, self._dict.tessellation, sphere)
    ...
    def save_as_fib(self, affine, voxel_size, output_file_name='output.fib.gz'):
        ...
        slice_size = [fib['dimension'][0] *
                      fib['dimension'][1], fib['dimension'][2]]

        tessellation_half_size = len(self._dict.tessellation.vertices) // 2

        try:
            orientation_agreement = np.array(
                nib.aff2axcodes(affine)) == self.FIB_COORDS_ORIENTATION
        except:
            warnings.warn(
                "Couldn't determine the orientation of coordinates from the affine.")
        ...
        output_odf = self._fib_reshape(
            self._odf, (voxels_num, tessellation_half_size), orientation_agreement)
        output_peak_dirs = self._fib_reshape(
            self._peak_dirs, (voxels_num, self._dict.max_peaks_num, 3), orientation_agreement)
        dict_idx = self._fib_reshape(
            self._dict_idx, voxels_num, orientation_agreement)

        # Resample output ODFs to the LPS coordinates
        if not np.all(orientation_agreement):
            fib_tessellation = Sphere(
                xyz=(2 * orientation_agreement.astype(int) - 1) *
                self._dict.tessellation.vertices,
        ...
        for j in range(voxels_num):
            ...
            peak_vertex_idx = np.mod(
                np.argmax(
                    np.dot(output_peak_dirs[j, peaks_filter], fib_tessellation.vertices.T), axis=1),
                tessellation_half_size
            )
            peak_vertex_values = output_odf[j][peak_vertex_idx]

            for i in range(len(peak_vertex_idx)):
                fib['index%d' % i][j] = np.mod(
                    peak_vertex_idx[i], tessellation_half_size)
                fib['fa%d' % i][j] = peak_vertex_values[i] - \
                    np.min(output_odf[j])

        for i in range(self._dict.max_peaks_num):
            fib['fa%d' % i] -= np.nanmin(fib['fa%d' % i])
            fib['nqa%d' % i] = fib['fa%d' % i] / \
                np.maximum(1e-8, np.nanmax(fib['fa%d' % i]))
        ...
        fib['D_iso'] = self._fib_index_map(
            self._dict.micro[self._dict.MICRO_DE, 0], dict_idx, slice_size)
        fib['p_iso'] = self._fib_index_map(
            self._dict.ratio[0], dict_idx, slice_size)
        ...
        for i in range(1, self._dict.max_peaks_num+1):
            ...
            for index_name in index_maps:
                fib[index_name] = self._fib_index_map(
                    index_maps[index_name], dict_idx, slice_size, fa_filter)

        for i in range(self._dict.max_peaks_num):
            fib['fa%d' % i] = self._fib_reshape(fib['fa%d' % i], slice_size)
            fib['nqa%d' % i] = self._fib_reshape(fib['nqa%d' % i], slice_size)
            fib['index%d' % i] = np.int16(
                self._fib_reshape(fib['index%d' % i], slice_size))

        output_file_prefix = output_file_name.replace(
            ".fib.gz", "").replace(".fib", "")
